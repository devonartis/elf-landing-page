...
export function Advisors() {
  return (
    <Grid sx={{ alignContent: 'center', justifyContent: 'center' }}>
      ...
      <Flex
        mt={['80px', '120px']}
        sx={{ justifyContent: 'center', flexWrap: 'wrap', mb: '300px', maxWidth: '700px' }}
        p={[1, 2]}
      >
        {team.map(({ file, name, twitter, role }, idx) => (
          ...
          >
            ...
            >
              <Image src={`/advisors/${file}`} layout="intrinsic" width="251px" height="269px" />
            </Box>
            <Grid gap={'6px'} sx={{ maxWidth: '175px' }}>
              <Text variant="text.subHeading" sx={{ fontWeight: 'heading', textOverflow: 'wrap' }}>
                {name}
              </Text>
              <Text variant="text">{role}</Text>
              <Link variant="nav" href={`https://twitter.com/${twitter}`} target="_blank">
